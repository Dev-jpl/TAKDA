...
function OutputCard({ output, onDelete }) {
  ...
  const getIcon = () => {
    if (isPlan) return <Kanban size={18} color={colors.modules.kalay} weight="light" />;
    if (isReport) return <FileText size={18} color={colors.modules.kalay} weight="light" />;
    return <FilePdf size={18} color={colors.modules.kalay} weight="light" />;
  };
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardFooter}>
        ...
        <TouchableOpacity style={styles.viewBtn}>
          ...
          <CaretRight size={12} color={colors.modules.kalay} weight="bold" />
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default function KalayOutputsTab({ userId }) {
  ...
  const loadOutputs = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await kalayService.getOutputs(userId);
      setOutputs(data);
    } catch (e) {
  ...
  const handleDelete = (id) => {
    Alert.alert('Erase Output', 'This action will permanently delete this unit from your repository.', [
      ...
      { 
        ...
        onPress: async () => {
          try {
            await kalayService.deleteOutput(id);
    ...
    return (
      <View style={styles.empty}>
        ...
        <Text style={styles.emptySub}>
          Ask Kalay to synthesize a report or mission plan to begin building your professional repository.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={loading} onRefresh={loadOutputs} tintColor={colors.modules.kalay} />
        }
        renderItem={({ item }) => (
