...
from services.ai import get_streaming_ai_response, search_chunks, get_ai_response_async
...
async def generate_session_title(conversation_history: list) -> str:
    """Generates a concise (3-5 words) title based on chat history."""
    history_snippet = "\n".join([
        f"{m['role']}: {m['content'][:200]}" 
        for m in conversation_history[-3:]
    ])

    system = "You are Kalay's summarizer. Based on the coordination history, create a 3-5 word catchy mission title. NO quotes, NO conversational filler."
    user = f"Chat History:\n{history_snippet}\n\nCreate mission title:"
    
    try:
        from services.ai import get_ai_response_async
        title = await get_ai_response_async(system, user)
