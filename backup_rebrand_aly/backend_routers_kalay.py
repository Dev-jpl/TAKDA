...
async def update_session(session_id: str, body: SessionUpdate):
    res = supabase.table("kalay_sessions").update({
        "title": body.title,
        "updated_at": "now()"
...
async def chat(body: KalayChatRequest, background_tasks: BackgroundTasks):
    ...
    async def stream_generator():
        ...
        if is_new_session:
            from services.kalay_agent import generate_session_title
            # We can't use complete history yet because we just saved the current message
            # But we have 'body.message' and 'clean_reply'
            history = [
...
async def async_update_title(session_id: str, history: list):
    ...
    supabase.table("kalay_sessions").update({"title": new_title}).eq("id", session_id).execute()
