...
export default function KalayQuizScreen({ route, navigation }) {
  ...
  const loadQuiz = async () => {
    try {
      const data = await kalayService.getQuiz(quizId);
      setQuiz(data.quiz);
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const res = await kalayService.submitQuiz(quizId, userId || '0000', answers);
      setResult(res);
      setSubmitted(true);
    } catch (e) {
  ...
  if (loading && !quiz) {
    return (
      <View style={styles.loading}>
        <ActivityIndicator size="large" color={colors.modules.kalay} />
      ...
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.resultContainer}>
          ...
          <TouchableOpacity style={styles.finishBtn} onPress={() => navigation.goBack()}>
            <Text style={styles.finishBtnText}>RETURN TO KALAY</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  optionBtnActive: {
    borderColor: colors.modules.kalay,
