  ...
  return (
    ...
    >
      <View style={styles.rowIcon}>
        <ChatCircleText size={18} color={colors.modules.kalay} weight="light" />
      </View>
...
export default function KalayHistoryTab({ userId, onResume, onNewChat }) {
  ...
  const loadSessions = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await kalayService.getSessions(userId);
      setSessions(data);
    } catch (e) {
  ...
  const submitRename = async () => {
    ...
    try {
      await kalayService.updateSession(editingSession.id, newTitle.trim());
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await kalayService.deleteSession(id);
  ...
  if (loading && sessions.length === 0) {
    return (
      <View style={styles.centered}>
        <ActivityIndicator color={colors.modules.kalay} />
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={loading} onRefresh={loadSessions} tintColor={colors.modules.kalay} />
        }
        renderItem={({ item }) => (
          <SessionRow 
            session={item} 
      ...
      <View style={styles.floatingControls}>
        ...
        <TouchableOpacity style={styles.minNewBtn} onPress={onNewChat} activeOpacity={0.7}>
          <Sparkle size={20} color={colors.modules.kalay} weight="fill" />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  minNewBtn: {
    ...
    backgroundColor: colors.modules.kalay + '25',
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 0.5,
  ...
  rowIcon: {
    ...
    height: 32,
    borderRadius: 10,
    backgroundColor: colors.modules.kalay + '10',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 12,
  },
